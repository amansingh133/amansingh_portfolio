    ...
    <section id="experience" style={{ position:'relative' }}>
      <div className="container">
        ...
        >
          ...
          <p className="section-subtitle">From intern to distributed systems engineer — the journey so far.</p>
        </motion.div>

        <div style={{ display:'grid', gridTemplateColumns:'1fr 2fr', gap:48, alignItems:'start' }} className="exp-grid">
          {/* Tab list */}
            ...
            {CONFIG.experience.map((exp, i) => (
              <motion.button key={i}
                ...
                style={{
                  ...
                  background: expanded===i ? 'var(--accent-glow)' : 'transparent',
              ...
              >
                <span style={{ fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:'0.9rem',
                  color: expanded===i ? 'var(--accent)' : 'var(--text)', transition:'color 0.2s' }}>
                  {exp.company}
                </span>
                ...
                {exp.current && (
                  <span style={{ marginTop:4, display:'inline-flex', alignItems:'center', gap:5,
                    fontSize:'0.65rem', color:'var(--accent)', fontFamily:"'Fira Code',monospace" }}>
          ...
          <AnimatePresence mode="wait">
            {CONFIG.experience.map((exp, i) => expanded === i && (
              <motion.div key={i}
                initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-20 }}
                transition={{ duration:0.35, ease:[0.22,1,0.36,1] }}
              >
                <div style={{ padding:'32px', background:'var(--card-bg)',
                  border:'1px solid var(--card-border)', borderRadius:20, backdropFilter:'blur(12px)' }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start',
                    flexWrap:'wrap', gap:12, marginBottom:24 }}>
                    <div>
                      <h3 style={{ fontFamily:"'Syne',sans-serif", fontSize:'1.3rem', fontWeight:800,
                        color:'var(--text)', marginBottom:4 }}>{exp.role}</h3>
                      <div style={{ fontFamily:"'Fira Code',monospace", fontSize:'0.82rem', color:'var(--accent)' }}>
                    ...
                    <span style={{ padding:'6px 14px', borderRadius:999,
                      ...
                      color:'var(--accent)', whiteSpace:'nowrap' }}>{exp.period}</span>
                  </div>
                  ...
                  <ul style={{ display:'flex', flexDirection:'column', gap:12, marginBottom:24 }}>
                    {exp.bullets.map((b, bi) => (
                      ...
                      >
                        <span style={{ color:'var(--accent)', marginTop:6, flexShrink:0, fontSize:'0.5rem' }}>◆</span>
                        <span style={{ color:'var(--text-muted)', lineHeight:1.75, fontSize:'0.92rem' }}>{b}</span>
                      </motion.li>
                    ))}
                  </ul>
